		...
		<div class="page-content">
			...
			<c:if test="${schedule.reportFlag ne 1 && schedule.reportFlag ne 0}">
				<c:if test="${schedule.reportFlag eq 2}">
					<h3 class="part3 text-center"><span style="color: red;">${gpms:getUser().name}</span>同学，很遗憾，你<strong>${schedule.content}报告</strong>未通过！<small>（请修改后重新提交）</small></h3>
				</c:if>
				<c:if test="${schedule.reportFlag eq 3}">
					<h3 class="part3 text-center">恭喜<span style="color: red;">${gpms:getUser().name}</span>同学，你<strong>${schedule.content}报告</strong>通过，现在开始准备你下一个步骤吧！</h3>
				</c:if>
				<div class="table_a center-block">
					<div class="page-title">${schedule.content}报告</div>
					<div class="table_1 table-responsive table-custom ">
						<table class="table table-hover table-bordered table-striped">
							...
							<tr>
								...
								<td>${schedule.sort}</td>
								<td>${fn:substringBefore(schedule.uploadFileOldName, ".")}</td>
								<td><fmt:formatDate value="${schedule.updateDate}" pattern="yyyy年MM月dd日" /></td>
								<td>
								...
								<td>
									<a href="${ctx}/student/downloadSchedule?id=${schedule.id}&checkStr=${schedule.checkStr}" class="btn btn-xs btn-primary">下载</a>
								</td>
							</tr>
							</tbody>
						</table>
				...
				<c:if test="${schedule.reportFlag eq 2}">
					<div class="submit_btn center-block">
						<a href="${ctx}/student/uploadSchedulePage?id=${schedule.id}" class="btn btn-info col-lg-12" >重新提交</a>
					</div>
				</c:if>
				</div>
			</c:if>
