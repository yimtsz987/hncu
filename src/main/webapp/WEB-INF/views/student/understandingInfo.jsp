...
<head>
	...
	<body>
		<div class="page-content">
			...
			<c:if test="${understandingInfo.passFlag ne 0}">
				<c:if test="${understandingInfo.passFlag eq 1}">
					<h3 class="part3 text-center"><span style="color: red;">${gpms:getUser().name}</span>同学，很遗憾，你<strong>审题报告</strong>未通过！<small>（请修改后重新提交）</small></h3>
				</c:if>
				<c:if test="${understandingInfo.passFlag eq 2}">
					<h3 class="part3 text-center">恭喜<span style="color: red;">${gpms:getUser().name}</span>同学，你<strong>审题报告</strong>通过，现在开始准备你下一个步骤吧！</h3>
				</c:if>
				<div class="table_a center-block">
					<div class="page-title">审题报告</div>
					<div class="table_1 table-responsive table-custom ">
						<table class="table table-hover table-bordered table-striped">
							...
							<tr>
								...
								<td>${understandingInfo.id}</td>
								<td>${fn:substringBefore(understandingInfo.uploadFileOldName, ".")}</td>
								<td><fmt:formatDate value="${understandingInfo.uploadDate}" pattern="yyyy年MM月dd日" /></td>
								<td>
								<span class="label label-success label-custom">
										${gpms:getDictLabel(understandingInfo.passFlag, 'understanding')}
								</span>
								</td>
								<td>
					...
					<div class="teacher_proposal">
						...
						<div class="teacher_proposal_text">
							${understandingInfo.teacherAdvise}
						</div>
					</div>
				<c:if test="${understandingInfo.passFlag eq 1}">
					<div class="submit_btn center-block">
						<a href="${ctx}/student/understanding?id=${understandingInfo.id}" class="btn btn-info col-lg-12" >重新提交</a>
					</div>
				</c:if>
				</div>
			</c:if>
