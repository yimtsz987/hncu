	...
	<body>
		<div class="page-content">
			<div class="page-search">
				...
				<form:form id="searchForm" modelAttribute="teacherOpenTitle" action="${ctx}/teacher/openTitleList" method="post" cssClass="form-horizontal">
					<input id="pageNum" name="pageNum" type="hidden" value="${teacherOpenTitlePageInfo.pageNum}"/>
					<input id="pageSize" name="pageSize" type="hidden" value="${teacherOpenTitlePageInfo.pageSize}"/>
					...
					<div class="form-group pull-left col-lg-3">
						...
						<div class="col-lg-9">
							<form:select path="openTitle.reportFlag" cssClass="form-control" id="reportFlag">
			...
			<div class="table-responsive table-custom">
				<table class="table table-hover table-bordered table-striped">
					...
					<tbody  id="tbodyId">
					<c:forEach items="${teacherOpenTitlePageInfo.list}" var="openTitle">
						<tr>
							<td><input type="checkbox" name="node"/></td>
							<td>${openTitle.user.student.node}</td>
							<td>${openTitle.user.name}</td>
							<td>${openTitle.user.student.titleName}</td>
							<td>
								<span class="label label-success label-custom">
									${gpms:getDictLabel(openTitle.openTitle.reportFlag, 'understanding')}
								</span>
							</td>
							<td>
								<c:if test="${openTitle.openTitle.reportFlag eq 0}">
									<a href="${ctx}/teacher/openTitleCheck?id=${openTitle.openTitle.id}"  class="btn btn-xs btn-danger" >审核</a>
								</c:if>
								<c:if test="${openTitle.openTitle.reportFlag ne 0}">
									<a href="${ctx}/teacher/openTitleCheck?id=${openTitle.openTitle.id}"  class="btn btn-xs btn-primary" >查看审核</a>
								</c:if>
							</td>
						</tr>
					</c:forEach>
			...
			<div class="page-info">
				${gpms:pageStr(teacherOpenTitlePageInfo)}
			</div>
		</div>
	</body>
<script type="text/javascript">
