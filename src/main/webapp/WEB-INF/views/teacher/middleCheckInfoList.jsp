	...
	<body>
		<div class="page-content">
			<div class="page-search">
				...
				<form:form id="searchForm" modelAttribute="teacherMiddleCheck" action="${ctx}/teacher/middleCheckInfoList" method="post" cssClass="form-horizontal">
				...
				<table class="table table-hover table-bordered table-striped">
					...
					<c:forEach items="${teacherMiddleCheckPageInfo.list}" var="middleCheck">
						<tr>
							...
							<td>
								<c:if test="${middleCheck.middleCheck.state eq 1}">
									-
								</c:if>
								<c:if test="${middleCheck.middleCheck.state eq 2}">
									${middleCheck.middleCheck.teacherAdvise}
								</c:if>
								<c:if test="${middleCheck.middleCheck.state eq 3}">
									-
								</c:if>
							</td>
							<td>
								<a href="${ctx}/teacher/downloadMiddleCheck?id=${middleCheck.middleCheck.id}&checkStr=${middleCheck.middleCheck.checkStr}" class="btn btn-xs btn-primary">下载</a>
							</td>
						</tr>
					</c:forEach>
					</tbody>
