	...
	<body>
		<div class="page-content">
			<div class="page-search">
				...
				<form:form id="searchForm" modelAttribute="teacherSchedule" action="${ctx}/teacher/scheduleList" method="post" cssClass="form-horizontal">
					...
					<div class="form-group pull-left col-lg-3">
						...
						<div class="col-lg-9">
							<form:select path="user.student.step5" cssClass="form-control" id="state">
				...
				<table class="table table-hover table-bordered table-striped">
					...
					<c:forEach items="${teacherSchedulePageInfo.list}" var="schedule">
						<tr>
							...
							<td>${schedule.user.student.node}</td>
							<td>${schedule.user.name}</td>
							<td>${schedule.user.student.titleName}</td>
							<td>
								<span class="label label-success label-custom">
										${gpms:getDictLabel(schedule.user.student.step5, 'step')}
								</span>
							</td>
							<td>
								<a href="${ctx}/teacher/scheduleInfoList?schedule.reportId=${schedule.schedule.reportId}&schedule.studentId=${schedule.user.id}"  class="btn btn-xs btn-primary" >查看详情</a>
							</td>
						</tr>
					</c:forEach>
					</tbody>
